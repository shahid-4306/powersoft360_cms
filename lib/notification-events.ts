...
export async function notifyComplaintCreated(params: {
  ...
  await notify({
    ...
    // Send admins straight to the Task & Complaint Assignment screen with
    // this complaint selected, instead of the generic reports page, so
    // clicking the notification puts the complaint directly in front of
    // whoever needs to process it.
    actionUrl: `/dashboard/tasks/assign?complaintId=${params.complaintId}`,
    recipients: {
...
/** A complaint was rejected by an Administrator from the Task &
 *  Complaint Assignment screen (invalid/incomplete submission). */
export async function notifyComplaintRejected(params: {
  ...
  await notify({
    ...
    message: `Complaint ${params.complaintNumber} was rejected by an Administrator.`,
    module: "complaints",
