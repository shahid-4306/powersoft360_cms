...
export const useCompanyInformation = () => {
  ...
  const importCompaniesFromXlsx = async (file: File) => {
    try {
      const body = new FormData();
      body.append("file", file);

      const response = await fetch("/api/company_information/import", {
        method: "POST",
        body,
      });
      ...
      // Refresh the list so newly imported companies appear immediately.
