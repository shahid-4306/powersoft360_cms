...
import { sendComplaintAssignmentEmail } from "@/lib/email-service";
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    // Expected Completion Time / Response Time is required so every
    // assigned complaint always carries a clear deadline the assigned
    // user and the customer can both see.
    if (!expectedCompletionAtRaw) {
      return NextResponse.json(
        {
          message: "Expected Completion Time is required to assign a complaint",
        },
        { status: 400 },
      );
    }

    const expectedCompletionAt = new Date(expectedCompletionAtRaw);
    if (isNaN(expectedCompletionAt.getTime())) {
      return NextResponse.json(
        {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (!allowedTypes.includes(file.type) && !allowedByExt) {
        console.warn(
          `Skipped file: ${originalName} - disallowed type: ${file.type}`,
        );
        continue;
      }
      ...
      if (file.size > 10 * 1024 * 1024) {
        console.warn(
          `Skipped file: ${originalName} - too large: ${file.size} bytes`,
        );
        continue;
      }
      ...
      try {
        ...
        console.log(
          `Uploading file ${i + 1}/${files.length}: ${originalName} (${file.size} bytes)`,
        );
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          uploadStream.on("finish", () => {
            console.log(
              `✅ File uploaded successfully: ${originalName} (ID: ${uploadStream.id})`,
            );
            resolve();
          });
    ...
    console.log("✅ Complaint assigned successfully:", {
      ...
      assignmentAttachmentsCount:
        updatedComplaint.assignmentAttachments?.length || 0,
    ...
    // a hardcoded/static address — so the customer knows their
    // complaint is now being worked on (non-blocking).
    if ((complaint as any).submitterEmail) {
      try {
        await sendComplaintAssignmentEmail(
          (complaint as any).submitterEmail,
          updatedComplaint.complaintNumber,
          (complaint as any).contactPerson,
          name || username,
          remarks,
        );
      } catch (emailError) {
        console.error("Complaint assignment email failed to send:", emailError);
      }
    }
    ...
    return NextResponse.json({
      ...
      complaint: {
        ...updatedComplaint,
        assignedDate: updatedComplaint.assignedDate
          ? updatedComplaint.assignedDate.toISOString()
          : null,
        expectedCompletionAt: updatedComplaint.expectedCompletionAt
          ? updatedComplaint.expectedCompletionAt.toISOString()
          : null,
        updatedAt: updatedComplaint.updatedAt
