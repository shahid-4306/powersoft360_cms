...
// . app/api/company_information/[id]/route.ts
import { NextResponse } from "next/server";
import dbConnect from "@/lib/db";
import CompanyInformation from "@/models/CompanyInformation";
import { isValidActiveSoftwareType } from "@/lib/softwareTypes";
import { findDuplicateCompany, normalizeText } from "@/lib/companyValidation";
...
export async function PUT(
  ...
  try {
    ...
    // If Company ID or Company Name are being changed, re-run duplicate
    // prevention (excluding this company itself) so an edit can never
    // collide with another existing company.
    const code =
      body?.code !== undefined ? normalizeText(body.code) : undefined;
    const companyName =
      body?.companyName !== undefined
        ? normalizeText(body.companyName)
        : undefined;
    ...
    if (code || companyName) {
      ...
      if (duplicate) {
        return NextResponse.json(
          {
            error: `Another company already uses this ${
              code && duplicate.code.toLowerCase() === code.toLowerCase()
                ? "Company ID"
                : "Company Name"
            } ("${duplicate.companyName}"). Duplicate companies are not allowed.`,
            duplicate: true,
